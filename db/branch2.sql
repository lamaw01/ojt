...
CREATE DEFINER=`root`@`localhost` PROCEDURE `eraseDashCoreLoan` ()  BEGIN
	UPDATE coreln
    	SET account_no = replace(account_no, '-','');
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `eraseDashCoreSavings` ()  BEGIN
	UPDATE coresv
    	SET account_no = replace(account_no, '-','');
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `eraseDashCoreTimeDeposit` ()  BEGIN
	UPDATE coretd
    	SET account_no = replace(account_no, '-','');
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `minus100MbwinLoan` ()  BEGIN
	UPDATE mbwinln
    	SET grantedamtorig = grantedamtorig / 100,
       	    grantedamt = grantedamt / 100,
            principal_amt = principal_amt / 100,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `minus100MbwinSavings` ()  BEGIN
	UPDATE mbwinsv
    	SET bal_amt = bal_amt / 100,
        acrbintamt = acrbintamt / 100,
        cumintpdamt = cumintpdamt / 100,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `minus100MbwinTimeDeposit` ()  BEGIN
	UPDATE mbwintd
    	SET bal_amt = bal_amt / 100,
        minperbalamt = minperbalamt / 100,
        int_bal_amt = int_bal_amt / 100;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateMigratedLoan` ()  BEGIN
	UPDATE migratedln
    	SET account_no = replace(account_no, "'",0),
        loan_amount = loan_amount / 100,
        interest_balance_amount = interest_balance_amount / 100,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateMigratedSavings` ()  BEGIN
	UPDATE migratedsv
    	SET account_no = replace(account_no, "'",''),
		current_balance = current_balance / 100,
        available_balance = available_balance / 100,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateMigratedTimeDeposit` ()  BEGIN
	UPDATE migratedtd
    	SET account_no = replace(account_no, "'",0),
		current_balance = current_balance / 100,
        available_balance = available_balance / 100,
